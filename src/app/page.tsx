...
export default function HomePage() {
  ...
  const visibleSiteElements = (settings.siteElements || []).filter((element) => element.visible);
  const featureElements = visibleSiteElements.filter((element) => element.placement === "homepage-feature");
  const workflowElements = visibleSiteElements.filter((element) => element.placement === "homepage-workflow");
    ...
    <main className="page-shell">
      ...
      <section className="hero-stage panel" data-reveal="fade" data-parallax="0.03">
        ...
        <div className="hero-visual" data-reveal="right" data-parallax="0.03">
          <div className="hero-logo-card" data-parallax="0.12">
            ...
            <Image
              src="/branding/logo.png"
              alt={`${settings.brandName} metallic K logo`}
              className="hero-logo"
              width={320}
              height={320}
              priority
          ...
          <div className="feature-grid">
            {featureElements.map((element, index) => (
              <article
                ...
                data-reveal={index % 3 === 0 ? "left" : index % 3 === 1 ? "fade" : "right"}
                data-parallax="0.025"
                key={element.id}
              >
                <span className="feature-tag">{element.eyebrow}</span>
                <h3>{element.title}</h3>
      ...
      {workflowElements.length ? (
        <section id="workflow" className="section-block workflow-grid">
          {workflowElements.map((element, index) => (
            <article
              className="panel"
              data-reveal={index % 3 === 0 ? "left" : index % 3 === 1 ? "fade" : "right"}
              data-parallax="0.02"
              key={element.id}
            >
              <span className="step-index">{element.eyebrow}</span>
              <h3>{element.title}</h3>
              <p>{element.description}</p>
            </article>
          ))}
      ...
      <section id="contact" className="section-block contact-banner panel" data-reveal="fade" data-parallax="0.03">
        <div>
          ...
          <p>
            To keep the studio inbox clean, project details and session files open after checkout.
            Pick the closest tier first, then send the notes with your upload.
          </p>
        </div>
        <div className="contact-links">
          <Link href="/mixing-mastering">Pay Then Send Project Details</Link>
